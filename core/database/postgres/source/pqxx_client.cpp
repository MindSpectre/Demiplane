...
namespace demiplane::database {
    ...
    Result PqxxClient::drop_connect() {
        ...
        result.critical_zone(
            [&] {
                conn_->close();
                TRACE_INFO(i_tracer, "Connection closed.");
            },
            [&](const std::exception& e) {
                TRACE_ERROR(i_tracer, "Failed to close connect. Required additional diagnostics.");
                return analyze_exception(e);
            });
        return result;
    }
    ...
    Result PqxxClient::start_transaction() {
        ...
        result.capture(
            [&] {
                std::lock_guard lock(conn_mutex_);
                if (in_transaction_) {
                    TRACE_DEBUG(i_tracer, "Transaction already started.");
                ...
                TRACE_DEBUG(i_tracer, "Transaction started.");
            },
            [&](const std::exception& e) {
                TRACE_DEBUG(i_tracer, "Failed to start transaction.");
                return analyze_exception(e);
            });
        return result;
    }
    ...
    Result PqxxClient::commit_transaction() {
        ...
        result.capture(
            [&] {
                std::lock_guard lock(conn_mutex_);
                if (!in_transaction_) {
                    TRACE_DEBUG(i_tracer, "Transaction already committed/rollbacked or not started.");
                ...
                TRACE_DEBUG(i_tracer, "Transaction committed.");
            },
            [&](const std::exception& e) {
                TRACE_DEBUG(i_tracer, "Failed to commit transaction.");
                return analyze_exception(e);
            });
    ...
    Result PqxxClient::rollback_transaction() {
        ...
        result.capture(
            [&] {
                std::lock_guard lock(conn_mutex_);
                if (!in_transaction_) {
                    TRACE_DEBUG(i_tracer, "Transaction already committed/rolled back or not started.");
                    throw TransactionException(
                ...
                open_transaction_.reset();
                in_transaction_ = false;
                TRACE_DEBUG(i_tracer, "Transaction rolled back.");
            },
            [&](const std::exception& e) {
                TRACE_DEBUG(i_tracer, "Failed to rollback transaction.");
                return analyze_exception(e);
            });
        return result;
    }
