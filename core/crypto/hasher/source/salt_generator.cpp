...
std::string demiplane::crypto::SaltGenerator::generate_base64(const std::size_t size) {
    const auto salt_bytes = generate_bytes(size);
    return encode_base64(salt_bytes);
}
std::string demiplane::crypto::SaltGenerator::encode_base64(const std::vector<std::uint8_t>& data) {
    if (data.empty()) {
        return {};
    }
