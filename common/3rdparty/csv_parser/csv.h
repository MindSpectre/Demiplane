...
namespace rapidcsv {
...
#else
    static constexpr bool sPlatformHasCR = false;
#endif
    ...
    public:
         ...
         */
        explicit Converter(const ConverterParams& pConverterParams) : mConverterParams(pConverterParams) {}
         ...
         */
        static void ToStr(const T& pVal, std::string& pStr) {
     ...
     */
    template <>
    inline void Converter<std::string>::ToStr(const std::string& pVal, std::string& pStr) {
        pStr = pVal;
    }
    ...
    struct SeparatorParams {
        ...
        explicit SeparatorParams(const char pSeparator = ',', const bool pTrim = false,
            const bool pHasCR = sPlatformHasCR, const bool pQuotedLinebreaks = false, const bool pAutoQuote = true,
    ...
    struct LineReaderParams {
        ...
        explicit LineReaderParams(
            const bool pSkipCommentLines = false, const char pCommentPrefix = '#', const bool pSkipEmptyLines = false)
    ...
    public:
         ...
         */
        explicit Document(std::string  pPath = std::string(), const LabelParams& pLabelParams = LabelParams(),
         ...
         */
        [[nodiscard]] int GetColumnIdx(const std::string& pColumnName) const {
            if (mLabelParams.mColumnNameIdx >= 0) {
                if (mColumnNames.contains(pColumnName)) {
        ...
        void InsertColumn(const size_t pColumnIdx, const std::vector<T>& pColumn = std::vector<T>(),
            ...
            for (auto itRow = mData.begin(); itRow != mData.end(); ++itRow) {
                if (std::distance(mData.begin(), itRow) >= mLabelParams.mColumnNameIdx) {
                    const auto rowIdx = static_cast<size_t>(std::distance(mData.begin(), itRow));
        ...
        [[nodiscard]] size_t GetColumnCount() const {
            const int count =
                static_cast<int>((!mData.empty()) ? mData.at(0).size() : 0) - (mLabelParams.mRowNameIdx + 1);
         ...
         */
        [[nodiscard]] int GetRowIdx(const std::string& pRowName) const {
            if (mLabelParams.mRowNameIdx >= 0) {
                if (mRowNames.contains(pRowName)) {
         ...
         */
        template <typename T>
        [[nodiscard]] std::vector<T> GetRow(const size_t pRowIdx) const {
         ...
         * @returns column name.
         */
        [[nodiscard]] std::string GetColumnName(const size_t pColumnIdx) const {
            const size_t dataColumnIdx = GetDataColumnIndex(pColumnIdx);
            if (mLabelParams.mColumnNameIdx < 0) {
        ...
        void SetColumnName(const size_t pColumnIdx, const std::string& pColumnName) {
            ...
            const auto rowIdx = static_cast<size_t>(mLabelParams.mColumnNameIdx);
            if (rowIdx >= mData.size()) {
                mData.resize(rowIdx + 1);
            }
            if (auto& row = mData[rowIdx]; dataColumnIdx >= row.size()) {
         ...
         * @returns row name.
         */
        [[nodiscard]] std::string GetRowName(const size_t pRowIdx) const {
            const size_t dataRowIdx = GetDataRowIndex(pRowIdx);
            if (mLabelParams.mRowNameIdx < 0) {
        ...
        void SetRowName(const size_t pRowIdx, const std::string& pRowName) {
            ...
            if (auto& row = mData[dataRowIdx]; mLabelParams.mRowNameIdx >= static_cast<int>(row.size())) {
         ...
         */
        [[nodiscard]] std::vector<std::string> GetRowNames() const {
            std::vector<std::string> row_names;
            if (mLabelParams.mRowNameIdx >= 0) {
                for (auto itRow = mData.begin(); itRow != mData.end(); ++itRow) {
                    if (std::distance(mData.begin(), itRow) > mLabelParams.mColumnNameIdx) {
                        row_names.push_back(itRow->at(static_cast<size_t>(mLabelParams.mRowNameIdx)));
                    }
                }
            }
            return row_names;
    ...
    private:
        ...
        void ParseCsv(std::istream& pStream, std::streamsize p_FileLength) {
            constexpr std::streamsize bufLength = 64 * 1024;
                ...
                for (size_t i = 0; i < static_cast<size_t>(readLength); ++i) {
                    if (buffer[i] == mSeparatorParams.mQuoteChar) {
                        if (cell.empty() || (cell[0] == mSeparatorParams.mQuoteChar)) {
                            ...
                            if (const auto firstQuote =
                                    std::ranges::find(cell, mSeparatorParams.mQuoteChar);
                                std::all_of(cell.begin(), firstQuote, [](const int ch) { return isspace(ch); })) {
                                quoted = !quoted;
                            }
                        }
                        cell += buffer[i];
            ...
            for (const auto & itr : mData) {
                for (auto itc = itr.begin(); itc != itr.end(); ++itc) {
                    if (mSeparatorParams.mAutoQuote
                        ...
                        std::string str                = *itc;
                        const auto quoteCharStr = std::string(1, mSeparatorParams.mQuoteChar);
        ...
        [[nodiscard]] size_t GetDataRowCount() const {
            return mData.size();
        }

        [[nodiscard]] size_t GetDataColumnCount() const {
            const auto firstDataRow =
        ...
        [[nodiscard]] size_t GetDataRowIndex(const size_t pRowIdx) const {
            const auto firstDataRow =
                static_cast<size_t>((mLabelParams.mColumnNameIdx + 1 >= 0) ? mLabelParams.mColumnNameIdx + 1 : 0);
            return pRowIdx + firstDataRow;
        }

        [[nodiscard]] size_t GetDataColumnIndex(const size_t pColumnIdx) const {
            const auto firstDataColumn =
                static_cast<size_t>((mLabelParams.mRowNameIdx + 1 >= 0) ? mLabelParams.mRowNameIdx + 1 : 0);
            return pColumnIdx + firstDataColumn;
        }

        [[nodiscard]] std::string Trim(const std::string& pStr) const {
            if (mSeparatorParams.mTrim) {
                std::string str = pStr;

                // ltrim
                str.erase(str.begin(), std::ranges::find_if(str, [](const int ch) { return !isspace(ch); }));

                // rtrim
                str.erase(
                    std::find_if(str.rbegin(), str.rend(), [](const int ch) { return !isspace(ch); }).base(), str.end());
        ...
        [[nodiscard]] std::string Unquote(const std::string& pStr) const {
