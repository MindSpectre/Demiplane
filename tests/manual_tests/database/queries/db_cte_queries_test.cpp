...
protected:
    void SetUp() override {
        ...
        sales_schema = std::make_shared<Table>("sales");
    ...
    std::shared_ptr<Table> sales_schema;
...
TEST_F(CteQueryTest, CteUsedInMainQueryExpression) {
    auto high_earners =
        with("high_earners", select(emp_id, emp_name).from(employees_schema).where(emp_salary > lit(80000.0)));
    ...
    auto main_query = select(emp_name.as("employee_name").as_dynamic().set_context(high_earners.name()), sale_amount)
                          .from(high_earners)
                          .join(sales_schema)
                          .on(sale_employee_id == (emp_id.as_dynamic().set_context(high_earners.name())))
...
TEST_F(CteQueryTest, CteWithComplexJoinsExpression) {
    auto employee_sales_summary =
        with("employee_sales_summary",
             select(emp_name, emp_department, sum(sale_amount).as("total_sales"), count(sale_id).as("sale_count"))
                 .from(employees_schema)
