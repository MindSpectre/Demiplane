...
namespace demiplane::tracing {
    ...
    struct FileTracerConfig :  TracerConfig {

    };
    class FileTracer final : public Tracer {
    public:
        FileTracer() = default;
        ...
        explicit FileTracer(const FileTracerConfig& config) : Tracer(config) {}

    private:
        static std::string create_log_file_name(const std::string_view directory_path) {
            std::ostringstream oss;
            oss << directory_path << "log_" << utilities::chrono::Clock::current_time_dmy() << ".log";
            return oss.str();
        }
