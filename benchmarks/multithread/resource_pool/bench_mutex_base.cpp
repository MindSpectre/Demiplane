...
#include "common/mutex_queue_pool.hpp"
#include "common/pool_bench_main.hpp"

using namespace bench::pool;

using PoolT                        = MutexQueuePool<MockResource>;
...
namespace {
    ...
    auto strategy = [](PoolT& p) noexcept { return p.acquire_for(ACQUIRE_FOR_TIMEOUT); };
...
int main(int argc, char** argv) {
    ...
    for (const auto kind : MUTEX_COMPATIBLE_SCENARIOS) {
        const Scenario sc      = scenario(kind);
        const std::string name = make_bench_name("MutexBase_AcqFor_10us", sc.name);
