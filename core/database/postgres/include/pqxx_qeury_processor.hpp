...
public:
        ...
        if (q.has_order_by()) {
            ...
            for (const auto& order : q.get_order_by_clauses()) {
                if (!first)
                    oss << ", ";
                first = false;
                oss << escape_identifier(order.field->get_name())
    ...
    static QueryAndParams process_upsert(query::UpsertQuery q, bool with_returning = false) {
        std::ostringstream oss;
        pqxx::params params;
        uint32_t param_counter = 1;
    ...
    static QueryAndParams process_create(const query::CreateQuery& q) {
        ...
        for (const auto& col : q.get_columns()) {
            ...
            first = false;
            oss << escape_identifier(col.get_column_name()) << " "
                << col.get_sql_type_initialization();
        }
        oss << ");";
