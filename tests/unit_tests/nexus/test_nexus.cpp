...
// Helpers
struct Counter {
    static constexpr uint32_t nexus_id = 0xFFF;
...
struct Counter2 {
    int x{0};
    static constexpr auto nexus_id = 0xFFFD;
...
struct MoveOnly {
    static constexpr uint32_t nexus_id = 0xFFA;
...
TEST(Nexus, FactorySingleton) {
    Nexus nx;
    nx.register_factory<Counter>([](Nexus&) {
...
TEST(Nexus, ResetFlexOK) {
    Nexus nx;
    nx.register_factory<Counter2>([](Nexus&) {
        return std::make_shared<Counter2>();
    }, Flex{});
...
TEST(Nexus, ThreadSafetySingleton) {
    ...
    for (int i = 0; i < 16; ++i) {
        threads[i] = std::thread([&nx, &ptrs, i] {
            ptrs[i] = nx.spawn<Counter>();
        });
    }
    for (auto& t : threads) {
        t.join();
...
struct With {
    static constexpr uint32_t nexus_id = 0xBEEF;
};
