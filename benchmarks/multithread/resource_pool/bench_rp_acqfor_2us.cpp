...
constexpr auto ACQUIRE_FOR_TIMEOUT = std::chrono::microseconds{2};
...
namespace {
    ...
    auto strategy = [](PoolT& p) noexcept {
        return p.acquire_for(ACQUIRE_FOR_TIMEOUT);
    };
...
int main(int argc, char** argv) {
    ...
    for (const auto kind : FREE_REGION_SCENARIOS) {
        ...
        const std::string name = make_bench_name("RP_AcqFor_2us", sc.name);
