...
protected:
    ...
    void SetUp() override
    {
        ...
        pool.fill(pool_size, create_mock_database);
    }
...
{
    std::vector<std::shared_ptr<DbInterface>> interfaces;
...
{
    pool::DbInterfacePool customPool;
...
{
    pool::DbInterfacePool failingPool;

    // Custom factory function that returns nullptr to simulate failure
    auto failingFactory = []() -> std::unique_ptr<DbInterface>
    {
        return nullptr;
    };
...
TEST_F(DbInterfacePoolTest, TestMultiThreadedAcquireRelease)
{
    ...
    for (int i = 0; i < 10; ++i)
    {
        threads.emplace_back([&] {
            ...
            {
                auto db_interface = pool.acquire();
                if (db_interface == nullptr) throw std::runtime_error("db_interface is null");
...
TEST_F(DbInterfacePoolTest, TestMultiThreadedSafe)
{
...
{
    std::unique_ptr<DbInterface> nullInterface;
