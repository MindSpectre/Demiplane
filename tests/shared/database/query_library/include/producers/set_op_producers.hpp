...
struct QueryProducer<set_op::UnionAll> {
    static db::CompiledQuery produce(const TestSchemas& s, db::QueryCompiler& c) {
        ...
        auto completed_orders = select(s.orders().user_id).from(s.orders().table).where(s.orders().completed == true);
        auto pending_orders = select(s.orders().user_id).from(s.orders().table).where(s.orders().completed == false);
...
struct QueryProducer<set_op::UnionWithOrderBy> {
    static db::CompiledQuery produce(const TestSchemas& s, db::QueryCompiler& c) {
        ...
        auto active_users = select(s.users().name, s.users().age).from(s.users().table).where(s.users().active == true);
        auto senior_users = select(s.users().name, s.users().age).from(s.users().table).where(s.users().age > lit(50));
...
struct QueryProducer<set_op::UnionWithLimit> {
    static db::CompiledQuery produce(const TestSchemas& s, db::QueryCompiler& c) {
        using namespace db;
        // UNION with LIMIT
        auto small_orders = select(s.orders().id, s.orders().amount)
                                .from(s.orders().table)
                                .where(s.orders().amount < lit(100.0));
        auto large_orders = select(s.orders().id, s.orders().amount)
                                .from(s.orders().table)
                                .where(s.orders().amount > lit(500.0));
...
struct QueryProducer<set_op::MultipleUnions> {
    static db::CompiledQuery produce(const TestSchemas& s, db::QueryCompiler& c) {
        ...
        auto middle = select(s.users().name).from(s.users().table).where(s.users().age >= lit(25) && s.users().age < lit(50));
