...
namespace demiplane::multithread {
    ...
    class ResourcePool : gears::Immutable {
        ...
        static constexpr std::chrono::nanoseconds spin_budget{1000};  // 1 us

    public:
        using value_type = T;

        static constexpr std::size_t max_size   = MaxSize;
        static constexpr std::size_t word_count = (MaxSize + bits_per_word - 1) / bits_per_word;

        /// Construct a free-only pool (n_pinned == 0) — the common case.
        template <typename Factory>
            requires ResourceFactory<Factory&, T>
        explicit ResourcePool(const std::size_t n_free, Factory&& make)
            : ResourcePool{std::size_t{0}, n_free, std::forward<Factory>(make)} {
        }

        /// Construct a partitioned pool.
        template <typename Factory>
            requires ResourceFactory<Factory&, T>
        ResourcePool(const std::size_t n_pinned, const std::size_t n_free, Factory&& make)
            : n_pinned_{n_pinned},
              n_free_{n_free},
        ...
        [[nodiscard]] std::optional<Lease<T>> acquire_for(const std::chrono::nanoseconds timeout) noexcept {
            ...
            const auto spin_end = t0 + spin_budget;
