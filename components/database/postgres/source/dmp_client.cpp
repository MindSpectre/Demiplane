...
#include "../include/pqxx_client.hpp"
...
namespace demiplane::database {
    ...
    // --- Execution Helpers ---
    Result PqxxClient::execute_query(const std::string& query_string, const pqxx::params& params) const {
    ...
    Result PqxxClient::execute_query(const std::string& query_string) const {
        std::lock_guard lock(conn_mutex_);
        Result result;
        result.capture([&] {
            auto txn = initialize_transaction();
    ...
    Interceptor<pqxx::result> PqxxClient::execute_query_with_result(
        const std::string& query_string, const pqxx::params& params) const {
        std::lock_guard lock(conn_mutex_);
        Interceptor<pqxx::result> result;
        result.capture([&] {
            auto txn              = initialize_transaction();
    ...
    Interceptor<pqxx::result> PqxxClient::execute_query_with_result(const std::string& query_string) const {
        std::lock_guard lock(conn_mutex_);
        Interceptor<pqxx::result> result;
        result.capture([&] {
            auto txn              = initialize_transaction();
    ...
    // Result PqxxClient::install_trgm_extension() const {
    ...
    // --- Table Management ---
    Result PqxxClient::create_table(const query::CreateTableQuery& proposal) {
    ...
    Result PqxxClient::delete_table(std::string_view table_name) {
        std::ostringstream oss;
        oss << "DROP TABLE IF EXISTS " << escape_identifier(table_name) << ";";
        execute_query(oss.str());
    }

    Result PqxxClient::truncate_table(std::string_view table_name) {
        std::ostringstream oss;
        oss << "TRUNCATE TABLE " << escape_identifier(table_name) << ";";
        execute_query(oss.str());
    }

    Interceptor<bool> PqxxClient::check_table(std::string_view table_name) {
        std::ostringstream oss;
        ...
        if (execute_res.is_err()) {
            return Interceptor<bool>{execute_res};
        }
    ...
    Result PqxxClient::make_unique_constraint(std::string_view table_name, FieldCollection key_fields) {
        std::lock_guard lock(conn_mutex_);
    ...
    Result PqxxClient::setup_search_index(std::string_view table_name, FieldCollection fields) {
        if (fields.empty()) {
    ...
    Result PqxxClient::drop_search_index(std::string_view table_name) const {
        std::ostringstream oss;
    ...
    Result PqxxClient::remove_search_index(std::string_view table_name) {
        std::lock_guard lock(conn_mutex_);
        search_fields_[std::string(table_name)].clear();
        drop_search_index(table_name);
    }

    Result PqxxClient::restore_search_index(std::string_view table_name) const {
        try {
            std::ostringstream fts_query;
            create_fts_index_query(table_name, fts_query);
            execute_query(fts_query.str());
    ...
    Result PqxxClient::insert(query::InsertQuery&& query) {
    ...
    Result PqxxClient::upsert(query::UpsertQuery&& query) {
        auto qp = PqxxQueryProcessor::process(query);
        if (qp.has_captured()) {
            qp.rethrow();
        }
    ...
    Interceptor<Records> PqxxClient::insert_with_returning(query::InsertQuery&& query) {
        auto qp = PqxxQueryProcessor::process(query);
        if (qp.has_captured()) {
            qp.rethrow();
        }
    ...
    Interceptor<Records> PqxxClient::upsert_with_returning(query::UpsertQuery&& query) {
        auto qp = PqxxQueryProcessor::process(query);
        if (qp.has_captured()) {
            qp.rethrow();
        }
    ...
    Interceptor<Records> PqxxClient::select(const query::SelectQuery& conditions) const {
    ...
    Result PqxxClient::remove(const query::RemoveQuery& conditions) {
        auto qp = PqxxQueryProcessor::process(conditions);
        if (qp.has_captured()) {
            qp.rethrow();
        }
    ...
    Interceptor<uint32_t> PqxxClient::count(const query::CountQuery& conditions) const {
        auto qp = PqxxQueryProcessor::process(conditions);
        if (qp.has_captured()) {
            qp.rethrow();
        }
