...
namespace demiplane::database::pool {
    ...
    public:
        std::unique_ptr<interfaces::DbInterface> acquire(const std::chrono::milliseconds timeout)
        {
            ...
            std::unique_ptr<interfaces::DbInterface> obj = std::move(pool_.back().interface);
            pool_.pop_back();
            full_cv_.notify_one();
            return obj;
        }
        std::unique_ptr<interfaces::DbInterface> acquire()
        {
            std::unique_lock lock(mutex_);
            if (pool_.empty()) {
                return nullptr;
        ...
        std::unique_ptr<interfaces::DbInterface> safe_acquire() noexcept {
            std::unique_lock lock(mutex_);
        ...
        bool release(std::unique_ptr<interfaces::DbInterface>&& obj) {
        ...
        void safe_release(std::unique_ptr<interfaces::DbInterface>&& obj) noexcept {
            std::unique_lock lock(mutex_);
        ...
        std::unique_ptr<interfaces::DbInterface> lend() {
            std::lock_guard lock(mutex_);
            if (pool_.empty()) {
            ...
            std::unique_ptr<interfaces::DbInterface> obj = std::move(pool_.front().interface);
            pool_.pop_front();
            return obj;
        }
    ...
    private:
        ...
        public:
            ...
            std::unique_ptr<interfaces::DbInterface> interface;
            DbConnection(const DbConnection&) = delete;
            explicit DbConnection(std::unique_ptr<interfaces::DbInterface>&& interface)
