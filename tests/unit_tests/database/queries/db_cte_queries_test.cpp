...
TEST_F(CteQueryTest, CteUsedInMainQueryExpression) {
    ...
    auto main_query = select(emp_name.as("employee_name"), sale_amount)
                      .from(high_earners)
                      .join(sales_schema->table_name()).on(sale_employee_id == emp_id)
...
TEST_F(CteQueryTest, MultipleCteExpression) {
    ...
    auto main_query = select(emp_name, emp_department, "total_sales")
                      .from(active_employees)
                      .join(high_sales).on(sale_employee_id == emp_id);
